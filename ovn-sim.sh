...
sim_srcdir=/usr/share/openvswitch; export sim_srcdir
interactive=false
scripts=
...
EOF
            ...
            ;;
        *)
            (sim_setvars $1; shift; "$@") # execute in a new shell 
            ;;
    esac
}
export -f as
...
EOF
    ...
    if test $1 = main; then
        echo "do nothing";return 0 #as $1 ovs-vswitchd -vvconn $daemon_opts
    fi

    # Create database and start ovsdb-server.
    touch $sim_base/$1/.conf.db.~lock~
    as $1 ovsdb-tool create $sim_base/$1/conf.db "$sim_srcdir/vswitch.ovsschema"
    ...
    ip netns add $1
    ip netns exec $1 bash -c "as $1 ovs-vswitchd -vvconn $daemon_opts"
...
EOF
    ...
    local net=$1 bridge=$2 tunip=$3
   
    port=${sandbox}_$bridge
    port2=${bridge}_$net

   # add veth pair
    ip link add $port type veth peer name ${port2}
    ip link set ${port2} netns ${sandbox}

    echo as main ovs-vsctl  -- add-port $net "$port" 
    as main ovs-vsctl  -- add-port $net "$port" 

    echo "ip link set dev ${port} up "
    ip link set dev ${port} up 

    echo ip netns exec ${sandbox} "ip address add $tunip/24 dev ${port2} "
    ip netns exec ${sandbox} ip address add $tunip/24 dev ${port2} 

    echo ip netns exec ${sandbox} "ip link set dev ${port2} up "
    ip netns exec ${sandbox} ip link set dev ${port2} up 
    #echo ip netns exec ${sandbox} bash -c "ovs-vsctl -- add-port $bridge ${port2} "
    #ip netns exec ${sandbox} bash -c "ovs-vsctl -- add-port $bridge ${port2} "
}
export -f net_attach
...
EOF
    ...
    ovn_start_db nb "$nbdb_model" "$nbdb_servers" "$sim_srcdir"/ovn-nb.ovsschema
    ovn_start_db sb "$sbdb_model" "$sbdb_servers" "$sim_srcdir"/ovn-sb.ovsschema

    ovn-nbctl init
    ovn-sbctl init

    mkdir "$sim_base"/northd
    echo as northd ovn-northd --ovnnb-db="$OVN_NB_DB" --ovnsb-db="$OVN_SB_DB"  $daemon_opts
...
EOF
    ...
    echo ip netns exec $sandbox ovn-controller --detach --no-chdir --pidfile -vconsole:off -vsyslog:off --log-file
    #ip netns exec $sandbox ovn-controller --detach --no-chdir --pidfile -vconsole:off -vsyslog:off --log-file
